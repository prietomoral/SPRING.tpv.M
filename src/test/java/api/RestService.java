...
public class RestService {
	...
	public String loginAdmin() {
		TokenWrapper token = new RestBuilder<TokenWrapper>(URL).path(Uris.TOKENS).basicAuth("123456789", "admin")
	...
	public String registerAndLoginManager() {
		...
		TokenWrapper token = new RestBuilder<TokenWrapper>(URL).path(Uris.TOKENS)
				.basicAuth(Long.toString(manager.getMobile()), manager.getPassword()).clazz(TokenWrapper.class).post()
				.build();
		return token.getToken();
	}
