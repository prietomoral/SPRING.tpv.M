tpv.service('f09Service', ['$http', '$q', function ($http, $q) {
  "use strict";

  const urlBase = "http://localhost:8080/SPRING.tpv.M.1.2.0-SNAPSHOT/api/v0";
  const resource = "cashier_balances";
  var Base64 = {
  ...
  this.listCashierBalances = function () {
    let config = {
  ...
  this.updateCashierBalance = function (id, cashier_balance) {
    let config = {
      headers: {Authorization: 'Basic ' + Base64.encode(sessionStorage.token + ':')},
      method: 'PUT',
      url: urlBase + "/" + resource + "/" + id,
      data: {
        ...
        "totalChange": cashier_balance.total_change,
        "totalCheck": cashier_balance.total_check,
        "createdDate": cashier_balance.created_date
      }
    };
    return this.request(config);
  }
  ...
  this.isActualDate = function (date) {
    ...
    return (date[0] === (actualDate.getFullYear())
            && date[1] === (actualDate.getMonth() + 1)
            && date[2] === (actualDate.getDate()));
  }
}]);