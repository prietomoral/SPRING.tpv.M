...
tpv.factory('AlertFamilyService', ['$http', 'APP', '$q', AlertFamilyService]);


function AlertFamilyService($http, APP, $q) {
  'use strict';
  ...
  function edit(id, alertFamily) {
    return $http({
      headers : { Authorization: 'Basic ' + Base64.encode(sessionStorage.token + ':')},
      method: 'PUT',
      url: APP.apiUrl + '/alerts-family/' + id,
      data: {
        name: alertFamily.name,
